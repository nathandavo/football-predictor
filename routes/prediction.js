...
async function fetchStats(homeTeamId, awayTeamId) {
  try {
    ...
    const getRecentForm = async (teamId) => {
      ...
      // Return numeric goals scored/conceded for short summary
    ...
    const lastH2H = (h2hData?.response ?? []).slice(0, 1);
    ...
    const stats = {
      h2h: lastH2H,
      homeStats: { name: `Home Team ${homeTeamId}`, recentForm: homeForm },
      awayStats: { name: `Away Team ${awayTeamId}`, recentForm: awayForm }
    };
    ...
    return {
      h2h: [],
      homeStats: { name: 'Home', recentForm: [] },
      awayStats: { name: 'Away', recentForm: [] }
    };
  }
}
...
router.post('/free', auth, async (req, res) => {
  try {
    ...
    const prompt = `
You are a football analyst. Provide a concise prediction for the upcoming match.

Output the following in plain text, aligned and neat:

1. Score prediction (Home vs Away)
2. Win probability chart using bars (Home/Draw/Away)
3. Expected goals chart using bars
4. Recent 5-match form summary for both teams (do NOT show W/L/D letters, instead use "good/bad/average" summary)
5. Include a one-line short reasoning for form and how it might affect the game

Use real team names, align charts so they start at the same column, do not use colors.
